...
sources:
  ...
  globalsCssSha256: "aa73b4b6a950bc5ab0957e52ae1edaf9c9b7f16f623215eeea5949f2d2c9bfae"
...
typography:
  ...
  fontStacks:
    ...
    chosunMyeongjo: "'Chosun Myeongjo', Pretendard, -apple-system, BlinkMacSystemFont, 'Apple SD Gothic Neo', 'Noto Sans KR', 'Segoe UI', sans-serif"
    moneygraphy: "'Moneygraphy Rounded', Pretendard, -apple-system, BlinkMacSystemFont, 'Apple SD Gothic Neo', 'Noto Sans KR', 'Segoe UI', sans-serif"
    chosunGulim: "'Chosun Gulim', Pretendard, -apple-system, BlinkMacSystemFont, 'Apple SD Gothic Neo', 'Noto Sans KR', 'Segoe UI', sans-serif"
    paperlogy: "'Paperlogy', Pretendard, -apple-system, BlinkMacSystemFont, 'Apple SD Gothic Neo', 'Noto Sans KR', 'Segoe UI', sans-serif"
...
motion:
  ...
  settingsDrawerOpen: "280ms cubic-bezier(0.22, 1, 0.36, 1) both"
  settingsDrawerClose: "220ms cubic-bezier(0.4, 0, 1, 1) both"
...
components:
  uiPrimitives: [Badge, Button, Dialog, Drawer, Popover, ScrollArea, Select, Separator, Skeleton, Tooltip]
...
`body`는 `--font-user`를 사용한다. 기본 Pretendard stack과 선택 가능한 Maplestory OTF, Hancom MalangMalang, Chosun Myeongjo, Moneygraphy Rounded, Chosun Gulim, Paperlogy stack은 front matter에 source spelling 그대로 기록했다. Pretendard와 Paperlogy는 100–900, Maplestory OTF와 Hancom MalangMalang은 400/700, 조선명조·Moneygraphy Rounded·조선굴림은 400을 제공한다. 모든 `@font-face`는 `font-display: swap`이고 preload하지 않는다. Paperlogy는 공식 v1.001 TTF의 9개 굵기를 WOFF2로 변환해 로컬 사용한다. Moneygraphy Rounded는 수정·재배포 금지 조건에 따라 로컬 파일 없이 토스 공식 `static.toss.im` WOFF2를 직접 사용한다. 조선명조와 조선굴림은 배포 형태 유지 조건에 따라 공식 TTF 원본을 변환하거나 subset하지 않는다. `FontThemeSelect`의 목록 라벨은 각 후보의 실제 font stack을 적용해 선택 전에 미리보기를 제공하므로, 메뉴를 열면 후보 폰트 로드가 시작될 수 있다. 선택된 trigger와 본문은 실제 `--font-user` stack을 사용한다.
...
본문 읽기 설정은 `--reading-font-size`, `--reading-line-height`, `--reading-font-weight`를 Notion article과 명시적인 PostCard title reading surface에 적용한다. PostCard title은 선택한 폰트가 지원하는 굵기를 100부터 900까지 그대로 적용하고 1.375의 heading line-height 계층은 보존한다. 줄 간격 compact/spacious에서는 heading용 line-height 단계가 바뀐다. 저장값은 hydration 전에 검증해 반영하고, 선택한 폰트가 실제로 지원하지 않는 굵기는 가장 가까운 지원값으로 보정한다. 한 가지 굵기만 제공하는 폰트에서는 굵기 선택 control을 표시하지 않는다. Header와 settings shell은 Tailwind scale을 유지하고 component docs, emoticon, Resume는 local typography를 유지한다. Resume 문서는 global font choice를 상속하지 않는 fixed-output island다.
...
Overlay 계층은 Radix Dialog/Drawer/Popover/Select, component mobile sheet, settings Drawer, Notion lightbox가 각자의 z-index와 shadow를 유지한다. settings panel은 desktop에서 오른쪽 25rem Drawer, mobile에서 높이 88dvh의 Bottom Sheet로 전환한다. 반투명 background와 backdrop blur, 20px 상단 radius를 공유하고 header/footer를 고정한 채 본문만 독립 스크롤한다. component registry demo와 emoticon action sheet의 depth는 shared Surface로 강제 통합하지 않는다.
...
`src/components/ui`의 10개 primitive는 Badge, Button, Dialog, Drawer, Popover, ScrollArea, Select, Separator, Skeleton, Tooltip이다. Button과 Badge만 CVA variant contract를 가진다. Button은 8 variants(`default`, `secondary`, `outline`, `ghost`, `glass`, `iconGlass`, `subtle`, `danger`)와 6 sizes(`default`, `sm`, `lg`, `icon`, `iconSm`, `iconLg`)다. Badge는 6 variants(`default`, `secondary`, `outline`, `tag`, `series`, `count`)다.

Dialog, Drawer, Popover, Select는 Radix adapters다. Drawer는 Dialog의 focus trap, ESC, light dismiss, focus return, body scroll lock을 유지하면서 desktop의 오른쪽 slide-over와 mobile의 bottom-sheet geometry를 소유한다. root/trigger/portal/content/item semantics와 `data-state`, `data-side`, `data-disabled`, `data-highlighted`, `data-placeholder`에 시각 상태가 연결된다. Separator는 `data-orientation`, ScrollArea는 `data-slot="scroll-area"`와 OverlayScrollbars runtime attributes를 유지한다. Body, ScrollArea, settings Drawer는 `src/lib/scrollbars.ts`의 640ms auto-hide 설정과 `os-theme-ryonglog`를 공유한다. Button과 Badge는 각각 `data-slot="button"`, `data-slot="badge"`를 public styling hook으로 제공한다.

ColorPicker는 Radix Popover 기반의 불투명 6자리 HEX 전용 primitive다. 채도·명도 평면은 pointer drag와 arrow key를, 색조 slider는 pointer와 keyboard를 지원하며 변경값을 즉시 상위 theme state로 전달한다. 색조 rail은 RGB hue spectrum의 고정 stop을 사용한다. 브라우저 native `input[type=color]` popup과 최근 색상 저장은 사용하지 않는다. SegmentedControl은 실제 button과 `aria-pressed`를 유지하면서 하나의 indicator를 220ms transform으로 이동하고, system 또는 in-app reduced motion에서는 이동 transition을 제거한다.

ColorPicker의 채도·명도 평면은 별도 바깥 border나 outer focus ring을 두지 않는다. 기본 edge와 keyboard focus는 모두 radius 안쪽 inset rim으로 표시해 색상 면의 모서리가 끊겨 보이지 않게 한다. 글꼴 굵기 Select는 이름과 숫자를 분리하고 숫자는 shared Badge primitive의 compact secondary treatment로 trigger와 option에 동일하게 표시한다.
...
활성 Header route의 선형 SVG 아이콘은 기본 2px보다 조금 굵은 2.35px stroke를 사용해 aurora surface 위에서도 현재 위치가 명확하게 보이도록 한다. 비활성 아이콘과 fill 기반 아이콘의 원래 geometry는 바꾸지 않는다.
...
Component mobile sheet는 180ms exit와 260ms enter, settings panel은 220ms close와 280ms open, settings item은 160ms transition을 사용한다. Settings panel은 desktop에서 viewport의 위·아래·오른쪽으로부터 16px 떨어진 400px floating drawer이며 네 모서리가 20px다. mobile에서는 viewport 하단에 붙는 bottom sheet로 전환되어 위쪽 두 모서리만 20px다. Settings panel은 desktop에서 `translateX`, mobile에서 `translateY`와 opacity만 움직이고 overlay는 opacity만 사용한다. 설정 색상은 하나의 glass surface 안에 native color picker와 HEX 입력을 결합하고 추천 색상을 함께 제공하며 최근 색상 기록은 유지하지 않는다. `코드 블럭` section은 글꼴 뒤, 접근성 앞에 위치하고 고정 TypeScript sample을 실제 Shiki 토큰 경계와 색상으로 미리 계산한 preview를 선택 control 위에 표시한다. Lightbox는 overlay 220–240ms, panel 260–320ms, controls 260ms다. Copy feedback는 180–220ms다. Emoticon action/button motion은 workspace가 소유한다.

최종 `prefers-reduced-motion` override와 사용자가 켠 `data-reduced-motion='true'`는 post/card/hero, TOC, component sheet, lightbox, copy feedback의 animation/transition을 제거한다. Settings Drawer와 settings/header controls는 state comprehension을 위해 이동 없는 90ms fade를 유지한다. 이 90ms exception은 의도된 현재 동작이며 0ms로 정규화하지 않는다. `prefers-contrast: more`는 motion이 아니라 scrollbar token override다.
...
Runtime/external custom properties are contracts, not missing design tokens: Radix transform origins and trigger dimensions, OverlayScrollbars `--os-*`, Shiki `--shiki-*`, registry component variables such as duration/gap/angle, theme boot `--theme-*`, reading preference `--reading-*`, and inline geometry from client interaction code.
