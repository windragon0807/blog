...
test('header renders centered icon controls with GitHub link', async ({ page }) => {
  ...
  expect(activeAuroraMetrics.beforeTop).toBe('-1px')
  expect(activeAuroraMetrics.beforeRadius).toBe(activeAuroraMetrics.radius)
  expect(activeAuroraMetrics.afterTop).toBe('1px')
  expect(activeAuroraMetrics.afterRight).toBe('1px')
  expect(activeAuroraMetrics.afterBottom).toBe('1px')
  expect(activeAuroraMetrics.afterLeft).toBe('1px')
  expect(activeInnerRadius).toBeCloseTo(activeRadius - 2, 1)
  await componentsLink.hover()
  await expect(componentsLink).toHaveCSS('border-color', 'rgba(0, 0, 0, 0)')
